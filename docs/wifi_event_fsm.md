...
- 链接丢失判定：`consumeDisconnect()` 仅在 FSM 处于 `Connected` 后收到 DISC 才视为运行中掉线（`reconnectLinkLoss_=true`）；Boot 失败同样以 `armReconnect(..., /*linkLoss=*/true)` 武装（开机失败 = 从上电起的断链）。
...
- 触发条件只有两种：**手动**（编码器菜单 Web Setup）或 **NVS 无保存 SSID**（开机空配置直入 AP）；重连永不自动切 SoftAP（见上）。
- 进入后是**纯 `WIFI_AP`**（不是 `AP_STA`）。
