...
| 2 | **温补系数 k=−0.5 先别信**：实测 ~−0.10 ppm/°C。Holdover 中错 k 会反向加误差（ΔT 5 °C → 多 2 ppm → 5 min 多 10 ms）。建议一夜大温差标定后再用；标定前 Holdover 可接受 k=0（50 ppm 地板罩得住） | 本实测 §3 | 高 |
| 3 | **`/status` 增加 `tempRefC_` 与原始估频样本字段**，现场可区分「机制正确 rebase≈0」与「根本没算」 | 当前 `tempCorrPpm=0` 无法自证 | 中 |
