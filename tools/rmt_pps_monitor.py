#!/usr/bin/env python3
"""RMT PPS board monitor for fw v1.1.35 (GPS_PPS_RMT_EN=1).
...
PASS (default thresholds):
  - fwMark contains 1.1.35 (or --skip-fw)
...
def main() -> int:
    ...
    if not args.skip_fw and "1.1.35" not in str(fw):
        fail.append(f"fwMark want 1.1.35 got {fw}")
    if not has_ppsrmt:
